...
setup() {
  ...
  export REPO_DIR
  # NB: this test uses the user's real ~/.canton-builder (no isolation possible
  # since the Docker daemon is shared); cleanup happens in teardown.
}

teardown() {
  "$REPO_DIR/canton" builder reset --purge <<< "yes" >/dev/null 2>&1 || true
}

@test "start no flags brings up app-provider only (no app-user)" {
  "$REPO_DIR/canton" builder start
...
@test "start --with app-user brings up both builtins" {
  "$REPO_DIR/canton" builder start --with app-user
...
@test "validator add acme produces a working 4th validator" {
  "$REPO_DIR/canton" builder start
  "$REPO_DIR/canton" builder validator add acme
...
@test "stop preserves intent; start brings same shape back; data persists" {
  "$REPO_DIR/canton" builder start
  "$REPO_DIR/canton" builder validator add acme
  "$REPO_DIR/canton" builder validator stop acme
  "$REPO_DIR/canton" builder stop
  "$REPO_DIR/canton" builder start
  ...
  "$REPO_DIR/canton" builder validator start acme
  curl -fs http://localhost:5903/api/validator/readyz
}
...
@test "validator rm wipes everything for that custom" {
  "$REPO_DIR/canton" builder start
  "$REPO_DIR/canton" builder validator add acme
  "$REPO_DIR/canton" builder validator rm acme
  [ ! -d "$HOME/.canton-builder/validators/acme" ]
  [ ! -f "$HOME/.canton-builder/nginx-customs/acme.conf" ]
  run jq -e '.validators[] | select(.name=="acme")' "$HOME/.canton-builder/validators.json"
  [ "$status" -ne 0 ]
}
...
@test "reset preserves recipes, reset --purge wipes them" {
  "$REPO_DIR/canton" builder start
  "$REPO_DIR/canton" builder validator add acme
  echo "yes" | "$REPO_DIR/canton" builder reset
  [ -d "$HOME/.canton-builder/validators/acme" ]
  echo "yes" | "$REPO_DIR/canton" builder reset --purge
